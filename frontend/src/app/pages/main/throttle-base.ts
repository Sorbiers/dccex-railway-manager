...
export abstract class ThrottleControllerBase {
    ...
    protected dcc = inject(DccService);
    ...
    setTrainSpeed(train: Device, value: number): void {
        this.dcc.setThrottle(train.address, value);
    }

    onSpeedChange(train: Device, event: Event): void {
        const speed = (event.target as HTMLInputElement).value;
        this.setTrainSpeed(train, parseInt(speed));
    }
    ...
    /**
     * Per-loco "road" accent colour. The Device model has no colour field, so we
     * derive a stable colour from the address against a fixed palette.
     */
    private readonly accentPalette = [
        '#e36414', '#3a7d44', '#c1121f', '#11497e',
        '#8a1f2e', '#1f9e87', '#6a4c93', '#1982c4'
    ];

    getAccent(train: Device | undefined): string {
        if (!train) return this.accentPalette[0];
        const i = Math.abs(train.address || 0) % this.accentPalette.length;
        return this.accentPalette[i];
    }
    ...
    getHandleRotation(train: Device): number {
        ...
        // Sweep the needle proportionally to speed (0..100% → 0..60°),
        // signed by direction (forward right, reverse left).
        const magnitude = 60 * Math.min(1, (train.speed || 0) / 126);
        return state === 'FORWARD' ? magnitude : -magnitude;
    }
    ...
    protected readonly stateDefs: Record<string, TurnoutStateDef[]> = {
        left: [
